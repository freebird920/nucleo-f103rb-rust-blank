...
impl RCC {
    ...
    pub fn set_sys_clock_32MHz(&self) {
        unsafe {
            ...
            // 2. Configure PLL: HSI/2 as PLL source, and multiply by 8
            ...
            // 3. Enable PLL and wait until it's ready
            rcc_cr_val = rcc_cr.read_volatile();
            rcc_cr_val |= (1 << 24); // PLLON
            rcc_cr.write_volatile(rcc_cr_val);
            while (rcc_cr.read_volatile() & (1 << 25)) == 0 {} // Wait until PLLRDY
    
            // 4. Configure flash prefetch, instruction cache, data cache, and wait state
